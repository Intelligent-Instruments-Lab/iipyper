...
class PdPatcher:
    ...
    def init(self):
        ...
        self.patch_ids['send'] = self.osc_send(self.osc.host, self.osc.port, self.s_x, self.s_y+self.h*2)
        self.comment("Python → Pd", self.r_x, self.r_y)
        self.comment("===========", self.r_x, self.r_y+self.h/2)
        self.patch_ids['receive'] = self.osc_receive(self.client_port, self.r_x, self.r_y+self.h*2)
        self.s_x+=300
        self.r_x+=300
    ...
    def osc_receive_with_controls(self, x, y, path, parameters):
        ...
        # sliders
        y_off+=10
        slider_ids, float_ids, int_ids, tbf_ids, _y_off = self.sliders(x, y+y_off, parameters, "receive")
        y_off+=160

        # [s arg_name]
        y_off+=_y_off+75
        send_ids = [
            self.object("s "+self.path_to_snakecase(path)+'_'+p['label'][0:3], 
    ...
    def osc_send_with_controls(self, x, y, path, parameters):
        ...
        y_off+=35
        receive_ids = [
            self.object("r "+self.path_to_snakecase(path)+'_'+p['label'][0:3], 
                            x+i*self.param_width, 
                            y+y_off+(0 if i % 2 == 0 else 25)) 
            for i, p in enumerate(parameters)]
        y_off+=30
    ...
    def param_comments(self, x, y, params):
        ...
        for i, p in enumerate(params):
            ...
            y_off+=15
            comment_id2 = self.comment(f'{p["data"][0]} {p["min_val"]} {p_max}', x_i, y+y_off)
